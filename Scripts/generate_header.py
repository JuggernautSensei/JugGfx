"""Generates JugGfx/JugGfx.h, the single public entry header for JugGfx.

Base-filter headers (the ones JugGfx/pch.h already includes) load first, in
...
Run after adding/removing a header in JugGfx/JugGfx/.
"""
...
JUGGFX_ROOT = Path(__file__).resolve().parent.parent
JUGGFX_DIR = JUGGFX_ROOT / "JugGfx"
FILTERS_FILE = JUGGFX_ROOT / "JugGfx.vcxproj.filters"
PCH_FILE = JUGGFX_DIR / "pch.h"
OUTPUT_FILE = JUGGFX_DIR / "JugGfx.h"
...
def main() -> None:
    ...
    all_headers = {p.name for p in JUGGFX_DIR.glob("*.h")}
    all_headers.discard("pch.h")
    all_headers.discard("JugGfx.h")
