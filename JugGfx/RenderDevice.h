...
class RenderDevice
{
    JUG_CLASS(RenderDevice, NO_COPY, NO_MOVE)
...
public:
    explicit RenderDevice(bool _bEnableDebugLayer);
    ~RenderDevice();

    [[nodiscard]] static RenderDevice* GetInstance();
...
#define JUG_GFX_DESTROY(_handle)                            \
    ...
    {                                                       \
        jug::RenderDevice::GetInstance()->Destroy(_handle); \
