...
namespace
{
    RenderDevice*      g_pSingleton        = nullptr;
...
{
    JUG_ASSERT(!g_pSingleton, "RenderDevice instance already exists.");
    g_pSingleton = this;
...
RenderDevice::~RenderDevice()
{
    JUG_ASSERT(g_pSingleton == this, "RenderDevice instance is not the singleton.");
...
{
    JUG_ASSERT(g_pSingleton, "RenderDevice instance is not created yet.");
    return g_pSingleton;
}

size_t RenderDevice::ReportLiveObjects()
{
    size_t num = 0;
...
const GraphicsCaps& RenderDevice::GetCaps() const
{
    return m_caps;
}

const GraphicsStats& RenderDevice::GetStats() const
{
    return m_lastStats;
}
...
VertexBufferHandle RenderDevice::CreateVertexBuffer(
...
VertexBufferHandle RenderDevice::CreateDynamicVertexBuffer(
...
InstanceBufferHandle RenderDevice::CreateInstanceBuffer(
...
IndexBufferHandle RenderDevice::CreateIndexBuffer(
...
IndexBufferHandle RenderDevice::CreateDynamicIndexBuffer(
...
ConstantBufferHandle RenderDevice::CreateConstantBuffer(
...
StorageBufferHandle RenderDevice::CreateStructuredBuffer(
...
StorageBufferHandle RenderDevice::CreateReadbackBuffer(
    const uint32_t _byteWidth)
{
...
StorageBufferHandle RenderDevice::CreateIndirectBuffer(
...
MutableMemoryView RenderDevice::MapBuffer(
    const BufferRef _buffer)
{
    const ResolveBufferRefResult resolve = ResolveBufferRef_(_buffer);
...
size_t RenderDevice::ReadBuffer(
...
TextureHandle RenderDevice::CreateTexture2D(
...
TextureHandle RenderDevice::CreateTextureCube(
    const uint32_t               _width,
    const uint32_t               _height,
    const eTextureFormat         _format,
...
TextureHandle RenderDevice::CreateTexture3D(
    const uint32_t               _width,
    const uint32_t               _height,
...
void RenderDevice::InitDebugInterfacesIfNeed_()
{
...
void RenderDevice::InitTimerQueries_()
{
...
void RenderDevice::CleanUpTimerQueries_()
{
...
RenderDevice::VertexBufferD3D11 RenderDevice::CreateVertexBuffer_(
...
RenderDevice::InstanceBufferD3D11 RenderDevice::CreateInstanceBuffer_(
    const uint32_t _numElems,
...
RenderDevice::IndexBufferD3D11 RenderDevice::CreateIndexBuffer_(
    const uint32_t   _numElems,
    const bool       _bU32,
    const bool       _bDynamic,
...
RenderDevice::ConstantBufferD3D11 RenderDevice::CreateConstantBuffer_(
...
RenderDevice::StorageBufferD3D11 RenderDevice::CreateStructuredBuffer_(
    const uint32_t                    _numElements,
    const uint32_t                    _stride,
    const Flags<eStorageBufferOption> _flags,
    const MemoryView                  _initDataOrEmpty) const
...
RenderDevice::StorageBufferD3D11 RenderDevice::CreateIndirectArgsBuffer_(
    const uint32_t                    _numDraws,
...
RenderDevice::StorageBufferD3D11 RenderDevice::CreateReadbackBuffer_(
    const uint32_t _byteWidth) const
{
...
void RenderDevice::ReleaseVertexLayout_(
...
VertexLayoutHandle RenderDevice::GetOrCreateVertexLayoutHandle_(
    const VertexLayout& _vl)
{
...
RenderDevice::ResolveBufferRefResult RenderDevice::ResolveBufferRef_(
    const BufferRef _buffer)
{
...
DXGI_SAMPLE_DESC RenderDevice::MakeSampleDesc_(const DXGI_FORMAT _format, const eMSAA _msaa) const
{
...
RenderDevice::TextureD3D11 RenderDevice::CreateTexture_(
    uint32_t               _width,
    uint32_t               _height,
    uint32_t               _depth,
    eTexture               _type,
...
void RenderDevice::UpdateTexture_(
...
void RenderDevice::UpdateTexture2D(
    const TextureHandle _texh,
    const uint32_t      _mip,
    const uint32_t      _layer,
    const uint32_t      _x,
...
void RenderDevice::UpdateTextureCube(
    const TextureHandle _texh,
    const uint32_t      _mip,
...
void RenderDevice::UpdateTexture3D(
    const TextureHandle _texh,
    const uint32_t      _mip,
    const uint32_t      _x,
    const uint32_t      _y,
...
size_t RenderDevice::ReadTexture(
...
FrameBufferHandle RenderDevice::CreateFrameBuffer(
...
void RenderDevice::ClearRenderTarget(
...
void RenderDevice::ClearRenderTargets(
    const FrameBufferHandle _fbh,
    const RGBA              _color)
{
    const FrameBufferD3D11& fb    = m_frameBufferPool[_fbh];
...
void RenderDevice::ClearDepthStencil(
    const FrameBufferHandle _fbh,
...
ID3D11InputLayout* RenderDevice::GetOrCreateD3d11InputLayout_()
{
    if (m_numStreams == 0)
    {
        return nullptr;
...
ProgramHandle RenderDevice::CreateProgram(
...
ProgramHandle RenderDevice::CreateComputeProgram(
...
void RenderDevice::PushDebugGroup(
...
void RenderDevice::PopDebugGroup() const
{
#ifdef JUG_DEBUG
    if (m_pUserAnnotationOrNull)
    {
...
void RenderDevice::SetDebugMarker(
    const StringView _name) const
{
#ifdef JUG_DEBUG
    if (m_pUserAnnotationOrNull)
...
void RenderDevice::SetVertexBuffers(
...
ID3D11SamplerState* RenderDevice::GetOrCreateSamplerState_(
...
ID3D11RasterizerState* RenderDevice::GetOrCreateRasterizerState_()
{
...
ID3D11BlendState* RenderDevice::GetOrCreateBlendState_()
{
...
ID3D11DepthStencilState* RenderDevice::GetOrCreateDepthStencilState_()
{
...
void RenderDevice::ApplyPipeline_()
{
...
void RenderDevice::SetIndexBuffer(
    const IndexBufferHandle _ibh,
    const uint32_t          _offset,
...
void RenderDevice::SetConstantBuffer(
    const ConstantBufferHandle _cbh,
...
void RenderDevice::SetTextureRW(
    const TextureHandle _texh,
...
void RenderDevice::SetRenderState(
    const Flags<eRenderState> _flags)
{
...
void RenderDevice::SetBlendFactor(
    const RGBA _factor)
{
...
void RenderDevice::SetComputeProgram(
    const ProgramHandle _ph)
{
    if (m_computePh == _ph)
    {
...
void RenderDevice::SetFrameBuffer(
    const FrameBufferHandle _fbh)
{
    if (m_fbh == _fbh)
    {
...
void RenderDevice::SetSubmitParam(
